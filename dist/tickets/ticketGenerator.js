...
exports.generateTicketLayout = generateTicketLayout;
 ...
 */
function hasFiveNumbersPerRow(ticket) {
...
function generateTicketLayout() {
    const ticket = Array.from({ length: ROWS }, () => Array(COLUMNS).fill(null));
    // Every column must contain at least one number.
    // We start with one number in every column.
    const columnCounts = Array(COLUMNS).fill(1);
...
function fillTicketNumbers(ticket) {
    for (let column = 0; column < COLUMNS; column++) {
